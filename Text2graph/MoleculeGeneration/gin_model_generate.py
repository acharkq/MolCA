...
class GCNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, edge_attr0=None, device=torch.device('cpu')):
        ...
        sl_attr = torch.ones(x.size(0), 1).to(device)
        sl_attr0 = torch.mm(sl_attr, self.edge_embedding1(torch.arange(4,5).to(device)))

        if edge_attr0 == None:
            edge_embeddings = self.edge_embedding1(edge_attr[:,0]) + self.edge_embedding2(edge_attr[:,1])
        else:
            edge1 = torch.mm(edge_attr0, self.edge_embedding1(torch.arange(4).to(device)))
            edge2 = torch.cat((edge1, sl_attr0), dim=0)
            # edge_embeddings = edge2 + self.edge_embedding2(edge_attr[:,1])
            # edge_embeddings = edge2
            if edge1.shape[0]==edge2.shape[0]:
                edge_embeddings = edge2 + self.edge_embedding2(edge_attr[:,1])
                ...
                print(edge1.shape, edge2.shape, self.edge_embedding2(edge_attr[:,1]).shape, edge_attr.shape)
                edge_embeddings = self.edge_embedding1(edge_attr[:,0]) + self.edge_embedding2(edge_attr[:,1]) #
            # edge_embeddings = torch.mm(edge_attr0, self.edge_embedding1(torch.arange(4).to(device))) + self.edge_embedding2(edge_attr[:,1])
        # edge_embeddings = torch.mm(edge_attr_temp, self.edge_embedding1) + self.edge_embedding2(edge_attr[:,1])
    ...
    def forward(self, x, edge_index, edge_attr, edge_attr0=None, device=torch.device('cpu')):
        ...
        if edge_attr0 == None:
            ...
            if edge1.shape[0]==edge2.shape[0]:
                ...
                print('Edge shape wrong!')
                print(edge1.shape, edge2.shape, self.edge_embedding2(edge_attr[:,1]).shape, edge_attr.shape)
                edge_embeddings = self.edge_embedding1(edge_attr[:,0]) + self.edge_embedding2(edge_attr[:,1]) #edge2
            # edge_embeddings = torch.mm(edge_attr0, self.edge_embedding1(torch.arange(4).to(device))) + self.edge_embedding2(edge_attr[:,1])
        # edge_embeddings = self.edge_embedding1(edge_attr[:,0]) + self.edge_embedding2(edge_attr[:,1])
        # edge_embeddings = torch.mm(edge_attr_temp, self.edge_embedding1) + self.edge_embedding2(edge_attr[:,1])
...
class GNN(torch.nn.Module):
    ...
    def forward(self, *argv):
        if len(argv) == 5:
            ...
            device = argv[4]
            # print(x.shape, x0.shape, edge_index.shape, edge_attr.shape, edge_attr0.shape, batch.shape)
        # elif len(argv) == 4:
        #     x, edge_index, edge_attr, batch = argv[0], argv[1], argv[2], argv[3]
        #     # print(x.shape, edge_index.shape, edge_attr.shape, batch.shape, x.dtype, edge_index.dtype, edge_attr.dtype, batch.dtype, x.type(), edge_index.type(), edge_attr.type(), batch.type())
        elif len(argv) == 1:
            data = argv[0]
            x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
        else:
