...
from model.bert import TextEncoder, MMEncoder
...
class GINSimclr(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.gtm:
            ...
            gtm_labels = torch.cat([torch.ones(bs, dtype=torch.long), torch.zeros(2 * bs, dtype=torch.long)], dim=0).to(self.device)
            loss_gtm = F.cross_entropy(gl_output, gtm_labels)
            self.log("gtm_loss", loss_gtm)
            loss += loss_gtm
...
#     cid = txt_name.split('_')[1].split('.')[0]
#     smiles = SMILES_dict[cid]
#     with open(os.path.join(txt_path, txt_name), 'r', encoding='utf-8') as t:
#         text = t.read()
#     with open(os.path.join(target_path, f"text_{cid}.txt"), 'w', encoding='utf-8') as f:
#         new_text=[]
#         for line in text.split('\n'):
#             try:
#                 new_line = line.replace(line[:line.index(' is')], smiles, 1)
#             except:
#                 new_line = line
#             new_text.append(new_line)
#         f.writelines(new_text)
#     with open(os.path.join(path, 'SMILES', f"smiles_{cid}.txt"), 'w', encoding='utf-8') as f:
